            ...
            if (CustomWinnerHolder.WinnerTeam != CustomWinner.Default)
            {
                ...
                if (CustomWinnerHolder.WinnerTeam is not CustomWinner.Draw and not CustomWinner.None)
                {
                    ...
                    foreach (var pc in Main.AllPlayerControls)
                    {
                        ...
                        if (pc.Is(CustomRoles.Yfox) && pc.IsAlive() && pc.GetPlayerTaskState().IsTaskFinished && !reason.Equals(GameOverReason.HumansByTask) && !reason.Equals(GameOverReason.ImpostorBySabotage))
                        {
                            CustomWinnerHolder.ResetAndSetWinner(CustomWinner.Yfox);
                            Main.AllPlayerControls
                                    .Where(p => p.Is(CustomRoles.Yfox) && !CustomWinnerHolder.WinnerIds.Contains(p.PlayerId))
                                    .Do(p => CustomWinnerHolder.WinnerIds.Add(p.PlayerId));
                        }
                        //SchrodingerCat
