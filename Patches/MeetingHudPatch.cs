        ...
        public static bool Prefix(MeetingHud __instance)
        {
            ...
            try
            {
                ...
                foreach (var pva in __instance.playerStates)
                {
                    ...
                    if (pc == null) continue;
                    //怪盗が投票済み
                    if (pc.Is(CustomRoles.PhantomThief) && pva.DidVote && pc.PlayerId != pva.VotedFor && pva.VotedFor < 253 && !pc.Data.IsDead)
                    {
                        var voteTarget = Utils.GetPlayerById(pva.VotedFor);
                        var TargetRoles = voteTarget.GetCustomRole();
                        if (TargetRoles.GetCustomRoleTypes() == CustomRoleTypes.Impostor)
                        {
                            pc.RpcExileV2();
                            pc.Data.IsDead = true;
                            Main.PlayerStates[pc.PlayerId].SetDead();
                            PhantomThief.NoticeDead();
                            return false;
                        }
                        pc.RpcSetCustomRole(TargetRoles);
                        PhantomThief.ChangeRole(voteTarget);
                        pc.SyncSettings();
                    }
                        //死んでいないディクテーターが投票済み
